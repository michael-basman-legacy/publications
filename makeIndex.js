...
async function generateHtmlIndex(pdfPaths) {
  const title = "Michael Basman Legacy Publications";
...
${listItems}
  ...
  await fs.promises.writeFile("docs/bareIndex.html", html, "utf8");
}
...
(async function main() {
  try {
    ...
    console.log("bareIndex.html has been written with", pdfs.length, "PDF(s).");
  } catch (err) {
